...
async function fetchCollection(collection, query) {
  try {
    ...
    const response = await fetch(
      `/api/collections/${collection}/entries?filter[title:contains]=${query}&filter[locale:is]=${siteLang.value}`,
      { signal }
    );
...
//   const collections = ["experience", "place", "pages"];
  try {
    ...
    // Combine results
    const allResults = [/*...experienceResults, ...placeResults, */...pagesResults];
